...
app.get("/", showReq, async (req, res) => {
  if (Object.keys(req.query).length === 0 || req.query.resto === "") {
    console.log("is empty");
    return res.json(defaultResto);
  }
  try {
    await axios
      .get(
        `https://lereacteur-bootcamp-api.herokuapp.com/api/deliveroo/menu/${req.query.resto}`,
        {
          headers: {
...
app.get("/exemple", showReq, (req, res) => {
  return res.json(defaultResto);
});
